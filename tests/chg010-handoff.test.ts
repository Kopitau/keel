...
test("REQ-012/AC-1 handoff guidance retains active scope and navigation without a line-count gate", () => {
  const skill = read(".agents/skills/k-handoff/SKILL.md");
  assert.match(skill, /Current goal and authorized scope/);
  assert.match(skill, /Relevant files/);
  assert.doesNotMatch(skill, /at most 10 lines|keel\/journal/i);
  assert.match(read("keel/handoff.md"), /keel\//);
});
...
test("REQ-012/AC-3 [proxy:independent harness recovery transcript not recorded] the recovery drill is recorded in the worklog with harness version date paths next step and tree hash", () => {
  ...
  for (const marker of ["harness/version", "date", "paths read", "recovered next step", "tree hash"]) {
    assert.match(skill, new RegExp(marker, "i"));
  }
  assert.match(skill, /without requiring a platform session file/);
});
...
test("REQ-012/AC-4 append-only worklog is sufficient to rebuild a skipped handoff", () => {
  ...
  assert.match(read(".agents/skills/k-handoff/SKILL.md"), /next agent rebuilds from the relevant worklogs/i);
});
