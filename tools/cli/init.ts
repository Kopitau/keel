...
import { runCheck } from "../gate/check.ts";
import { EXEC_REQUIRED } from "../gate/execmode.ts";
import { runSync } from "../gate/sync.ts";
import { writeTestBaseline } from "../gate/testbase.ts";
import { fail, ok, type CmdResult } from "../gate/result.ts";
import { INIT_COPY, flag, parseHuman, readInstallerVersion } from "./layout.ts";
...
export function looksLikeLegacyFramework(cwd: string): string[] {
  ...
  if (existsSync(sp)) {
    try {
      ...
      if (names.some((n) => /superpowers/i.test(n))) hints.push(".agents/skills/*superpowers*");
    } catch {
      /* ignore */
    }
  }
...
export function buildCleanConfig(opts: InitOpts): string {
  ...
  const body = {
    ...
    platforms: { primary, compatible: ["pi"] },
    budget: {
...
export function runInit(opts: InitOpts): CmdResult {
  const cwd = opts.cwd;
  ...
  const out = [
    `keel init ${opts.name || basename(cwd)}`,
    ...notes.map((n) => `note: ${n}`),
    chk.stdout.trim(),
    "",
  ].join("\n");
...
export function initFromArgs(cwd: string, source: string, args: string[]): CmdResult {
  const human = flag(args, "human");
  const parsed = human ? parseHuman(human) : { name: flag(args, "human-name") ?? "", email: flag(args, "human-email") ?? "" };
  const tier = flag(args, "tier") || "local";
  if (!["local", "github", "gitee"].includes(tier)) {
