...
发布日期：2026-09-01。补丁版。变更单 CHG-014（两个试点项目 zhaoxi / fmea-v3 的审计回流：DEC-185～189、ISS-058～060）；CHG-012 / CHG-013 / CHG-014 与需求 v6 已由 APR-006 一次批准（2026-09-01 用户原话；DEC-190 下由 agent 身份提交），requirements current = v6.md。
...
- **harness 探测**（ISS-059）：`Agent:` 会由 `CODEX_*` 环境变量或父进程名（codex / claude / opencode / grok / dsh）识别；Cursor / VS Code 只记 `Host:`。"approved APR 无 `delegated:` 即拒绝"的守卫在任何环境都触发（DEC-190，见下）；`KEEL_ANCESTRY=0` 关闭父进程探测，`KEEL_AGENT` 覆盖。
- **审批提交不再要求人类 git 身份**（DEC-190）：`gate approve` 不再核 git 身份，只要求 APR 前言有用户原话 `delegated:`（并填 `approver:`，默认 `identities.humans[0]`，可 `--approver`）；pre-commit 与 X-apr 也只核原话；agent 可用自己的身份直接提交审批。旧规则下"agent 身份提交审批"会红的仓库现在不红；没有原话的 approved APR 仍然红。
...
- **证据按功能说话**（REQ-006/AC-10）：`gate trace` 开头与 `gate verify` 结尾都打印「按功能」摘要——每个功能认领的需求标题、验收条目数、黑盒 / 替身 / 缺失各几条、测试文件；AGENTS.md 要求 agent 汇报证据时引用它而不是"N 条测试通过"。
