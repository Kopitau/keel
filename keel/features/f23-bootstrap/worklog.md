...
## 2026-09-06（0.12.2 GitHub 交付）

- 用户「推送到github」后，推送 cd3337c 至 Kopitau/keel 的 codex/safe-framework-update，设置同名上游；未强推、未合并或改变默认分支。第一次网络超时，重试成功，无需重新授权。
- 推送钩子复用 tree=7775d0cd114729613350af25917c831f6915dc61 的 319 项通过证据：F23 更新确认、无变化/非交互、指令边界和项目内容保护功能回归通过；完整检查无失败、两类既有提醒。实际远程 CI 的状态不由本地通过推断。

## 2026-09-06（0.12.2 / CHG-018 确认流程补漏）

- 用户确认「是的」：补掉无变化仍确认、非交互仍显示无法回答问题的遗漏。根因是 0.12.1 仅按 --yes 决定提问，没有先看实际操作是否为空，也没有在输出前判断终端能否确认。
- 修复：终端适配层显式提供 canConfirmUpdate；无变化跳过确认直接报告实际完成状态；非交互有变化仅预览和提示 --yes、零写入。交互终端保留预览后 y/N，指令 pending 仍返回 2，不扩张授权。
- 验证：更新回归修前 11 过 / 5 失败，修后 16 过 / 0 失败；新增真实 bin 的普通无变化更新、可交互适配器不应被无变化更新调用；保留实际覆盖、拒绝/EOF 零写入和项目规则字节保护。正式验证和消费项目实跑在后续条目记录。
- C-34: ref=CHG-018 把旧“非交互也应显示 y/N”的措辞断言改为不应提问，取消/零写入断言保留；无变化及 pending 的无写入路径不应再要求 --yes。未修改业务要求、框架权限或技能。
- 本机验证：taotie 尚需从 0.12.1 到 0.12.2 时，非交互普通 update 不显示提问、明确提示 --yes，全部文件和暂存区哈希不变。按既有授权执行 --yes 后，普通 update 返回 already up to date 0.12.2 / pending=none，无确认或取消文字；quick 无失败或提醒。更新仅涉及 config 的工具版本和 installed.json；其它 198 个文件、暂存区保持不变。
- 类型声明同步新增可选的 canConfirmUpdate；更新与发布回归 19 过 / 0 失败，tsc 退出 0。后续以干净提交运行正式完整验证。
- 完成：实施提交 7c4e11e328fb33e4cb932db218361676bc52d8c0；干净代码树 gate verify 退出 0，319 过 / 0 失败 / 0 跳过，tree=7775d0cd114729613350af25917c831f6915dc61，dirty=false。F23 的无变化、非交互、真实 bin、确认拒绝/EOF 零写入和 pending 状态回归均通过；完整 check 退出 0，无失败、两类既有提醒（远程 CI 替身、历史审批变动）。当前修复与 taotie 本机更新已完成，未推送 GitHub。
