...
即确认推荐 B：技能验收 v1 = 协议机检 + 发现层探测；**五家 harness 真人真机 live 实点不做**，如实记为未做（ISS-013 wontfix），不假装已实点。

**过程说明**：本决策在实施时曾把用户「修 P2」的指示当作按 B 实施的授权，属先做后问（C-21 边界）；状态标注诚实（proposed + 可否决），事后用户确认与实施一致。下次触及此类边界应先问。
