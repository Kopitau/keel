...
- Raw user intent and agent working interpretation are distinct. When useful, use `## 原始意图` / `## 工作解释` in requirements and plans, and `## 技术实现` for the selected mechanism. Preserve verbatim words, source and scope; never invent a user quote or treat an interpretation as approval. Frozen behavioral/acceptance semantics still require a new version and CHG. Current progress stays in handoff/evidence, not in the user's intent.
- For spec–implementation governance, incrementally declare exact repo-relative `implementation: []` and `related: []` paths in feature plans; a feature may govern multiple files and shared files may have multiple owners. Use `gate drift` to locate changed promises and implementation, and `gate atlas` for their offline visual projection. Follow `tools/gate/spec-governance.md` when adopting or recording a `drift review`. Unmapped is unknown, not aligned; resolve the actual mismatch and inspect functional evidence before recording a reasoned content review. A spec edit alone never proves alignment; review is not APR approval, test execution or user acceptance.
