...
Read `keel/templates/migrate/trellis.md`, `superpowers.md`, `unstructured.md`, and `report.md`. Run `node tools/gate/gate.ts status`.

## Rules

1. **Read-only source** (C-132). Snapshot source paths and bytes before and after migration. Do not delete or rewrite Trellis/Superpowers trees. After a successful mapping, add a standalone `keel-migrated` marker only; record that addition separately from the unchanged source-byte comparison.
...
4. Copy `keel/templates/migrate/report.md` and write a Chinese migration report with every category: mapped / not mapped / conflicts / doubts / unclassified / awaiting confirmation (C-133).
