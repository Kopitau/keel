...
**Keep going (DEC-183).** Implement → tests → one worklog line with the evidence → `gate check --quick` → next slice. When the feature's last slice passes: `gate verify`, compress the worklog into `summary.md` (four sections), run **k-retro** (OVERVIEW.md 能力清单 / 在途, provisional DECs, lesson candidates — G-done warns while OVERVIEW is older than the newest summary, CHG-016), refresh `keel/handoff.md` (≤10 lines), then start the next frontier feature from `gate status`. Before writing the summary, if the feature closes with zero ISS and zero `#经验候选`, re-read its worklog once: a problem that came back (same script, same workaround, same "again") is an ISS or a candidate, not a footnote (C-57/C-78; a pilot closed 13 features with none of either). A new context reads only summary / plan / handoff, never the compressed worklog. Stop only for C-21, a fused review, or final acceptance.
