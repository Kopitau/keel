...
1. `node tools/gate/gate.ts loop pack [--base <rev>] --implementer <h> --reviewer <other-h>` — five keys only: diff (since `--base`: deleted files by name only, 2 context lines), plan (the current overview; the feature `plan/vN.md` obligations it maps are part of it), reqs (REQ entries in scope), evidence (JSON from `gate verify`), worklog_summary (digest, never the chat). Extra fields, chat-shaped text or oversized blobs are refused (C-39).
