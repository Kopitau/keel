# Formal plan acceptance review

Read this reference only for a formal plan-level acceptance or an existing review loop. Ordinary code review uses the main skill and checklist; it does not require creating loop artifacts.

G-done's existing all-active-features-have-summary condition still requires this formal loop. A normal review result alone does not satisfy it; keep ordinary implementation completion and that plan-acceptance condition distinct.
...
The formal loop's ac-only route checks auto / machine-doc test-name mappings. A manual criterion is deferred for real evidence review and cannot be cleared by adding a test name. Do not manufacture a behavioral claim to satisfy it. A proxy remains WARN, not PASS; missing real manual validation is an explicit acceptance condition, not a demand for a fake automated test.
